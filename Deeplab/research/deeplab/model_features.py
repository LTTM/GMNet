...
def predict_labels_multi_scale(images,
  ...
  Args:
    ...
    macro_classes_features: A tensor of macro classes features
  ...
  """
    ...
    for i, image_scale in enumerate(eval_scales):
        with tf.variable_scope(tf.get_variable_scope(), reuse=True if i else None):
            outputs_to_scales_to_logits, edge, parts_features = multi_scale_logits(
                images,
                macro_classes_features,
        ...
        if add_flipped_images:
            with tf.variable_scope(tf.get_variable_scope(), reuse=True):
                outputs_to_scales_to_logits_reversed, edge, parts_features = multi_scale_logits(
                    tf.reverse_v2(images, [2]),
                    tf.reverse_v2(macro_classes_features, [2]),
                    model_options=model_options,
                    image_pyramid=[image_scale],
                    is_training=False,
                    fine_tune_batch_norm=False)
    ...
    return outputs_to_predictions, edge, parts_features


def predict_labels(images, macro_classes_features, model_options, image_pyramid=None):
    """Predicts segmentation labels.
    ...
    for output in sorted(outputs_to_scales_to_logits):
        ...
        if model_options.prediction_with_upsampled_logits:
            ...
            predictions[output + PROB_SUFFIX] = tf.image.resize_bilinear(
                ...
                align_corners=True,
                name='resize_prob')
    return predictions, edge, parts_features
...
def multi_scale_logits(images,
  ...
  """
    ...
    for image_scale in image_pyramid:
        ...
        outputs_to_logits, edge, parts_features = _get_logits(
            scaled_images,
            macro_classes_features,
            updated_options,
        ...
        if len(image_pyramid) == 1:
            ...
            return outputs_to_scales_to_logits, edge, parts_features
...
def extract_features(images,
  ...
  """
    ...
    parts_features = features

    if not model_options.aspp_with_batch_norm:
        return features, end_points, edge, parts_features
    else:
        if model_options.dense_prediction_cell_config is not None:
            ...
            with slim.arg_scope(
                ...
                with slim.arg_scope([batch_norm], **batch_norm_params):
                    ...
                    # TODO: verificare funzionamento concat features e macro_classes_features e edge_net
                    ...
                    if macro_classes_features is not None:
                        features = tf.concat([macro_classes_features, features], 3)
                    ##############################################
                    ...
                    return concat_logits, end_points, edge, parts_features
...
def _get_logits(images,
  ...
  """
    features, end_points, edge, parts_features = extract_features(
        images,
        macro_classes_features,
        model_options,
        weight_decay=weight_decay,
    ...
    return outputs_to_logits, edge, parts_features
