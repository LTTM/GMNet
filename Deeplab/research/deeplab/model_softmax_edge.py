...
def predict_labels_multi_scale(images,
  ...
  Args:
    ...
    softmax: A tensor of size [batch, height, widthm 21]
...
def predict_labels(images, softmax, model_options, image_pyramid=None):
    """Predicts segmentation labels.

  Args:
    images: A tensor of size [batch, height, width, channels].
    softmax: A tensor of size [batch, height, width, 21].
    model_options: A ModelOptions instance to configure models.
        ...
        if model_options.dense_prediction_cell_config is not None:
            ...
            with slim.arg_scope(
                ...
                with slim.arg_scope([batch_norm], **batch_norm_params):
                    ...
                    # TODO: verificare funzionamento simple_net
                    kernel_init = 'he_uniform'
                    use_batch_norm = True
                    mult_rate = 1
                    dropout = False
                    simple_res = simple_net(softmax, model_options, weight_decay, reuse, kernel_init, use_batch_norm,
                                            is_training, fine_tune_batch_norm, mult_rate, dropout)

                    features = tf.concat([simple_res, features], 3)

                    # TODO: verificare funzionamento psp_net e edge_net
                    kernel_init_edge = 'he_uniform'
                    use_batch_norm_edge = True
                    edge, edge_fea = edge_net(end_points, model_options, weight_decay, reuse, kernel_init_edge,
                                              use_batch_norm_edge, is_training, fine_tune_batch_norm)
...
def simple_net(softmax, model_options, weight_decay, reuse, kernel_init, use_batch_norm, is_training,
               fine_tune_batch_norm, mult_rate=1, dropout=False):
    ...
    with slim.arg_scope(
        ...
        if use_batch_norm:
            with slim.arg_scope([batch_norm_fn], **batch_norm_params):
                with tf.variable_scope(SIMPLE_NET_SCOPE, SIMPLE_NET_SCOPE, [softmax]):
                    # conv1
                    simple_res = slim.conv2d(softmax,
                                             num_outputs=mult_rate * 128,
                                             kernel_size=7,
                                             stride=2,
                                             padding='SAME',
                                             scope='conv1')
                    ...
                    if dropout:
                        simple_res = slim.dropout(simple_res,
                                                  keep_prob=0.8,
                                                  seed=1,
                                                  is_training=is_training,
                                                  scope='conv1_dropout')
                    ...
                    # conv2
                    simple_res = slim.conv2d(simple_res,
                                             num_outputs=mult_rate * 256,
                                             kernel_size=7,
                                             stride=1,
                                             padding='SAME',
                                             scope='conv2')
                    ...
                    # pool1
                    simple_res = slim.max_pool2d(simple_res, kernel_size=3, stride=2, padding='SAME', scope='pool1')

                    # conv3
                    simple_res = slim.conv2d(simple_res,
                                             num_outputs=mult_rate * 512,
                                             kernel_size=5,
                                             stride=2,
                                             padding='SAME',
                                             scope='conv3')
