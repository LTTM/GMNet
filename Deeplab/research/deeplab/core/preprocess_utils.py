...
def randomly_scale_image_and_label_softmax(image, label=None, softmax=None, scale=1.0):
    """Randomly scales image and label.
  ...
  """
    ...
    if softmax is not None:
        softmax = tf.squeeze(tf.image.resize_bilinear(tf.expand_dims(softmax, 0), new_dim, align_corners=True), [0])

    return image, label, softmax


def randomly_scale_image_and_label_complete(image, label=None, softmax=None, gt_macro_classes=None, edges=None, scale=1.0):
    # No random scaling if scale == 1.
    if scale == 1.0:
        return image, label, softmax, gt_macro_classes, edges
    ...
    if gt_macro_classes is not None:
        gt_macro_classes = tf.image.resize(
            gt_macro_classes,
            new_dim,
            method=get_label_resize_method(gt_macro_classes),
            align_corners=True)
    ...
    if edges is not None:
        edges = tf.image.resize(
            edges,
            new_dim,
            method=get_label_resize_method(edges),
            align_corners=True)

    return image, label, softmax, gt_macro_classes, edges


def randomly_scale_image_and_label_multitasking(image, label=None, gt_classes=None, scale=1.0):
    """Randomly scales image and label.
  ...
  """
    ...
    if gt_classes is not None:
        gt_classes = tf.image.resize(
            gt_classes,
            new_dim,
            method=get_label_resize_method(gt_classes),
            align_corners=True)
...
def resize_to_range(image,
  ...
  """
    with tf.name_scope(scope, 'resize_to_range', [image]):
        ...
        new_tensor_list.append(tf.image.resize(image, new_size, method=method, align_corners=align_corners))
...
def resize_to_range_softmax(image,
                            label=None,
                            softmax=None,
                            min_size=None,
                            max_size=None,
  ...
  """
    with tf.name_scope(scope, 'resize_to_range', [image]):
        ...
        if softmax is not None:
            resized_softmax = tf.image.resize_bilinear(softmax, new_size, align_corners=align_corners)
            new_tensor_list.append(resized_softmax)

        return new_tensor_list


def resize_to_range_complete(image,
                             label=None,
                             softmax=None,
    ...
    with tf.name_scope(scope, 'resize_to_range', [image]):
        ...
        if gt_macro_classes is not None:
            if label_layout_is_chw:
                # Input gt_macro_classes has shape [channel, height, width].
                resized_gt_classes = tf.expand_dims(gt_macro_classes, 3)
                resized_gt_classes = tf.image.resize(
                    resized_gt_classes,
                    new_size,
                    method=get_label_resize_method(gt_macro_classes),
                    align_corners=align_corners)

                resized_gt_classes = tf.squeeze(resized_gt_classes, 3)

            else:
                # Input gt_macro_classes has shape [height, width, channel].
                resized_gt_classes = tf.image.resize(
                    gt_macro_classes,
                    new_size,
                    method=get_label_resize_method(gt_macro_classes),
            ...
            new_tensor_list.append(resized_gt_classes)
        ...
        if edges is not None:
            if label_layout_is_chw:
                # Input edges has shape [channel, height, width].
                resized_edges = tf.expand_dims(edges, 3)
                ...
                resized_edges = tf.squeeze(resized_edges, 3)

            else:
                # Input edges has shape [height, width, channel].
                resized_edges = tf.image.resize(
                    edges,
                    new_size,
                    method=get_label_resize_method(edges),
            ...
            new_tensor_list.append(resized_edges)
...
def resize_to_range_multitasking(image,
                                 label=None,
                                 gt_classes=None,
                                 min_size=None,
                                 max_size=None,
  ...
  """
    with tf.name_scope(scope, 'resize_to_range', [image]):
        ...
        if gt_classes is not None:
            if label_layout_is_chw:
                # Input gt_classes has shape [channel, height, width].
                resized_gt_classes = tf.expand_dims(gt_classes, 3)
                resized_gt_classes = tf.image.resize(
                    resized_gt_classes,
                    new_size,
                    method=get_label_resize_method(gt_classes),
