...
from research.deeplab.core import dense_prediction_cell
...
EDGE_NET_SCOPE = 'edge_net'
INTERP_BLOCK_SCOPE = 'interp_block'
PSP_NET_SCOPE = 'psp_net'
  ...
  """
    ...
    for i, image_scale in enumerate(eval_scales):
        with tf.variable_scope(tf.get_variable_scope(), reuse=True if i else None):
            outputs_to_scales_to_logits, edge = multi_scale_logits(
                images,
        ...
        if add_flipped_images:
            with tf.variable_scope(tf.get_variable_scope(), reuse=True):
                outputs_to_scales_to_logits_reversed, edge = multi_scale_logits(
    ...
    return outputs_to_predictions, edge
...
def multi_scale_logits(images,
  ...
  """
    ...
    for image_scale in image_pyramid:
        ...
        outputs_to_logits, edge = _get_logits(
            scaled_images,
            updated_options,
        ...
        if len(image_pyramid) == 1:
            ...
            return outputs_to_scales_to_logits, edge
        ...
        if model_options.dense_prediction_cell_config is not None:
            ...
            with slim.arg_scope(
                ...
                with slim.arg_scope([batch_norm], **batch_norm_params):
                    ...
                    # TODO: verificare funzionamento psp_net e edge_net
                    kernel_init = 'he_uniform'
                    use_batch_norm = True
                    edge, edge_fea = edge_net(end_points, model_options, weight_decay, reuse, kernel_init,
                                              use_batch_norm, is_training, fine_tune_batch_norm)

                    # psp_res = psp_net(features, model_options, weight_decay, reuse, kernel_init,
                    #                   is_training, fine_tune_batch_norm)
                    ...
                    return concat_logits, end_points, edge
...
def _get_logits(images,
  ...
  """
    features, end_points, edge = extract_features(
        images,
        model_options,
        weight_decay=weight_decay,
        reuse=reuse,
            ...
            with tf.variable_scope(DECODER_SCOPE, DECODER_SCOPE, [features]):
                ...
                for output_stride in decoder_output_stride:
                    ...
                    for i, name in enumerate(feature_list):
                        ...
                        if ('mobilenet' in model_variant or model_variant.startswith('mnas') or
                        ...
                        for j, feature in enumerate(decoder_features_list):
                            decoder_features_list[j] = _resize_bilinear(feature,
...
def interp_block(features, level, model_options, weight_decay, reuse, kernel_init,
                 is_training, fine_tune_batch_norm,):
    ...
    with slim.arg_scope(
            ...
            reuse=reuse):
        with slim.arg_scope([batch_norm_fn], **batch_norm_params):
            with tf.variable_scope(INTERP_BLOCK_SCOPE, INTERP_BLOCK_SCOPE, [features]):
                x = slim.avg_pool2d(features, level, scope='avg_level_' + str(level))
                x = slim.conv2d(x,
                                num_outputs=512,
                                kernel_size=1,
                                stride=1,
                                padding='SAME',
                                scope='conv_level_' + str(level))

                x = tf.image.resize_bilinear(x,
                                             size=tf.shape(features)[1:3],
                                             align_corners=True,
                                             name='resized_conv_level_' + str(level))

                return x


def psp_net(features, model_options, weight_decay, reuse, kernel_init, is_training,
    ...
    with slim.arg_scope(
            ...
            reuse=reuse):
        with slim.arg_scope([batch_norm_fn], **batch_norm_params):
            with tf.variable_scope(PSP_NET_SCOPE, PSP_NET_SCOPE, [features]):
                x1 = interp_block(features, 1, model_options, weight_decay, reuse, kernel_init, is_training,
                                  fine_tune_batch_norm)
                x2 = interp_block(features, 2, model_options, weight_decay, reuse, kernel_init, is_training,
                                  fine_tune_batch_norm)
                x3 = interp_block(features, 3, model_options, weight_decay, reuse, kernel_init, is_training,
                                  fine_tune_batch_norm)
                x6 = interp_block(features, 6, model_options, weight_decay, reuse, kernel_init, is_training,
                                  fine_tune_batch_norm)

                psps_concat_res = tf.concat([features, x1, x2, x3, x6], axis=-1, name='psp_concat')
                psp_res = slim.conv2d(psps_concat_res,
                                      num_outputs=512,
                                      kernel_size=3,
                                      stride=1,
                                      padding='SAME',
    ...
    # x1 = interp_block(x, num_filters=num_filters, level=1, output_stride=output_stride)
    # x2 = interp_block(x, num_filters=num_filters, level=2, output_stride=output_stride)
    # x3 = interp_block(x, num_filters=num_filters, level=3, output_stride=output_stride)
    # x6 = interp_block(x, num_filters=num_filters, level=6, output_stride=output_stride)
    #
    ...
    #
    # y = Concatenate(axis=bn_axis, name='psp_concat')([x, x1, x2, x3, x6])
    #
    # # y = _conv(filters=num_filters, kernel_size=(3, 3), padding='same', block='pyramid_out_' + str(output_stride))(y)
    # # y = _conv(filters=classes, kernel_size=(3, 3), padding='same', block='pyramid_out_' + str(output_stride))(y)
    # y = Conv2D(
    #     filters=classes,
    #     kernel_size=(3, 3),
    #     padding='same',
    ...
    # )(y)
    #
    # y = BatchNormalization(axis=bn_axis, name='bn_pyramid_out_' + str(output_stride))(y)
    # y = Activation('relu')(y)
    #
    # return y


def edge_net(end_points, model_options, weight_decay, reuse, kernel_init, use_batch_norm, is_training,
             fine_tune_batch_norm,):
    # TODO: mettere lo scope dell'encoder
    x1 = end_points['resnet_v1_101/' +
                    feature_extractor.networks_to_feature_maps[model_options.model_variant][feature_extractor.DECODER_END_POINTS][4][0]]
    x2 = end_points['resnet_v1_101/' +
                    feature_extractor.networks_to_feature_maps[model_options.model_variant][feature_extractor.DECODER_END_POINTS][8][0]]
    x3 = end_points['resnet_v1_101/' +
                    feature_extractor.networks_to_feature_maps[model_options.model_variant][feature_extractor.DECODER_END_POINTS][16][0]]
    ...
    with slim.arg_scope(
            ...
            normalizer_fn=None,
            reuse=reuse):
        with tf.variable_scope(EDGE_NET_SCOPE, EDGE_NET_SCOPE, [end_points]):
            # conv1
            with slim.arg_scope(
                    [slim.conv2d],
                    weights_regularizer=slim.l2_regularizer(weight_decay),
            ...
            edge2_fea = tf.image.resize_bilinear(edge2_fea,
                                                 size=tf.shape(x1)[1:3],
                                                 align_corners=True)

            edge3_fea = tf.image.resize_bilinear(edge3_fea,
                                                 size=tf.shape(x1)[1:3],
                                                 align_corners=True)

            edge2 = tf.image.resize_bilinear(edge2,
                                             size=tf.shape(x1)[1:3],
                                             align_corners=True)

            edge3 = tf.image.resize_bilinear(edge3,
                                             size=tf.shape(x1)[1:3],
                                             align_corners=True)

            edge = tf.concat([edge1, edge2, edge3], axis=-1)
            edge_fea = tf.concat([edge1_fea, edge2_fea, edge3_fea], axis=-1)

            edge = slim.conv2d(edge,
                               num_outputs=2,
                               kernel_size=1,
                               stride=1,
                               padding='SAME',
