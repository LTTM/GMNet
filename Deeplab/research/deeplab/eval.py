...
flags.DEFINE_integer('model_edge', 0, 'Use model_edge or not')
...
def main(unused_argv):
    ...
    print_message(FilesName.EVAL, "Start at " + date_now, MessageType.HEADER)
    ...
    with tf.Graph().as_default():
        ...
        samples = dataset.get_one_shot_iterator(use_softmax=1,
                                                use_gt_macro_classes=1,
                                                use_edges=1).get_next()
        ...
        if tuple(FLAGS.eval_scales) == (1.0,):
            ...
            if FLAGS.model_softmax == 1:
                predictions = model_softmax.predict_labels_multi_scale(
    ...
    date_now = datetime.now().strftime("%Y%m%d-%H%M%S")
    print_message(FilesName.EVAL, "End at " + date_now, MessageType.HEADER)
